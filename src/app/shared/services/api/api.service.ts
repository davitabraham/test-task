...
export class ApiService {
  ...
  getHeaders(body: any = null, token: boolean): HttpHeaders {
    const headersObj = {};
  ...
  request(type: string, url: string, body: object | null = null, token: boolean = true): Observable<any> {
    const to = `${this.BASE_API}/${url}`;

    const options = {
      observe: 'response',
      headers: this.getHeaders(body, token)
    };
